...
# Tính NAV
async def compute_nav(taikhoan: str) -> int:
    ndt = await db.nha_dau_tu.find_one({"taikhoan": taikhoan})
    if not ndt:
        return 0

    total_value = 0
    async for s in db.so_huu.find({"maNDT": ndt["taikhoan"]}):  # hoặc dùng ndt["_id"] nếu muốn
        maCP = s["maCP"]
...
async def get_watchlist(taikhoan: str) -> List[WatchlistItem]:
    ndt = await db.nha_dau_tu.find_one({"taikhoan": taikhoan})
    if not ndt:
        return []

    result = []
    async for s in db.so_huu.find({"maNDT": ndt["taikhoan"]}):
        maCP = s["maCP"]
        ...
        item = WatchlistItem(
            ...
            lich_su_gia=lich_su_gia(**lich_su) if lich_su else None
        )
        result.append(item)
    return result
